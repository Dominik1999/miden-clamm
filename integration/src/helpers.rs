...
/// Initializes client infrastructure against the local Phase 4 network
/// stack (see `local-net/start-stack.sh`; RPC on localhost:57291).
///
/// Uses separate store/keystore paths (`local-store.sqlite3`,
/// `local-keystore/`) so local validation never contaminates testnet state.
///
/// # Errors
/// Returns an error if RPC connection fails, keystore initialization fails,
/// or client building fails
